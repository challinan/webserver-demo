 ...
 *
 * $Id: cgi.c,v 1.1 2007/02/08 16:28:56 challinan Exp $
 */
...
/*
 *				We get here if the process has terminated.  Read the
 *				output file and send its contents to the socket.
 */
				wp = cgip->wp;

				fdout = gopen(cgip->stdOut, O_RDONLY | O_BINARY, 0444 );
				glseek(fdout, 0, SEEK_SET);
				websWrite(wp, T("HTTP/1.0 200 OK\r\n"));

				while ((n = gread(fdout, cgiBuf, FNAMESIZE)) > 0) {
					websWriteBlock(wp, cgiBuf, n);
				}
				websDone(wp, 200);

/*
 *				Close and remove the temporary re-direction files
 */
				gclose(fdout);
				gunlink(cgip->stdIn);
				gunlink(cgip->stdOut);
